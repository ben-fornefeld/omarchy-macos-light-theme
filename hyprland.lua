-- macOS 2026 "Tahoe" Light — Liquid Glass, ported in full from hyprland.conf.
--
...
-- gradient. So the border below is a plain translucent black hairline
-- instead of the previous multi-stop graphite gradient.
local hairline_active = "rgba(0000001A)" -- ~10% black
local hairline_inactive = "rgba(0000000F)" -- ~6% black
local shadow_active = "rgba(00000033)"
local shadow_inactive = "rgba(0000001C)"
...
hl.config({
  ...
  group = {
    ...
    groupbar = {
      ...
      rounding = 10,
      rounding_power = 4.0,
      text_color = "rgba(1C1C1EEE)",
      col = {
        active = { colors = { "rgba(8E8E9366)", "rgba(6E6E7366)" }, angle = 90 },
        inactive = "rgba(0000000F)",
      },
      gradients = true,
    },
  },
...
-- Bezier curves (same Apple-style springs as the dark variant).
...
hl.layer_rule({ name = "macos-light-bar", match = { namespace = "omarchy-bar" }, blur = true, ignore_alpha = 0.2 })
hl.layer_rule({
  name = "macos-light-menu",
  match = { namespace = "^(omarchy-menu|omarchy-image-selector|omarchy-emojis|omarchy-clipboard|omarchy-keyboard-panel)$" },
  blur = true,
  ignore_alpha = 0.2,
})
hl.layer_rule({ name = "macos-light-notifications", match = { namespace = "omarchy-notifications" }, blur = true, ignore_alpha = 0.2 })
hl.layer_rule({ name = "macos-light-osd", match = { namespace = "omarchy-osd" }, blur = true, ignore_alpha = 0.2 })
hl.layer_rule({ name = "macos-light-hyprpicker", match = { namespace = "hyprpicker" }, blur = true })
